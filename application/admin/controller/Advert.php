...
use app\admin\validate\Advert as validate;
use app\admin\model\Advert as model;

class Advert extends Index
{
	function _empty()
	{
    ...
    function index()
    {
        $list = model::field('id,name,sm_pic,status')->order('id','DESC')->paginate(10);
        $total = model::count();
    ...
    function edit($id)
    {
        $result = model::field('id,name,sm_pic,status,url')->find($id);
    ...
    function update(Request $request, $id)
    {
        if($request->isAjax()){
            ...
            if($image){
                ...
                $img_path = Db::name('advert')->field('sm_pic,pic')->find($id);
                // 处理图片
                if( !$fileName = $image->validate(['size'=>2097152,'ext'=>'jpeg,jpg,png,gif'])->move(ROOT_PATH . 'public_html' . DS . 'upload') )
                    return json( getMsg(0,$image->getError()) );
                else{
    ...
    function deletion(Request $request)
    {
        ...
        // 先找出图片路径
        $path = ROOT_PATH . 'public_html' . DS . 'upload' . DS;
        $img_path = Db::name('advert')->field('sm_pic,pic')->whereIn('id',$ids)->select();
